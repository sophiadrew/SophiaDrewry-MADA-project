...
Source: https://juliasilge.com/blog/cocktail-recipes-umap/
```{r}
# creating model type
lm.mod <- linear_reg() %>% 
            set_engine('lm') %>% 
...
```{r}
# worried a bit about multicollinearity, So I am going to run a few models with and without ENSO related measures. Because of this reason vairiable selection will be important
summary(nbr1 <- glm.nb(Total ~  Pop + Precip + TAvg + SOI + ENSO, data = train.dta, maxit = 50)) # TAvg does not perform well
...
It appears that this model found only Max and Min Air Tempertaure as predictors
## Visualize
```{r}
...
Sources: https://emilhvitfeldt.github.io/ISLR-tidymodels-labs/classification.html#k-nearest-neighbors
```{r}
# create recipe
...
LASSO | 113.3661	
K Nearest Forest Model | 61.2996
