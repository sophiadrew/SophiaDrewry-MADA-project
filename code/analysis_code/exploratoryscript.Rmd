...
CP1 = ggplot(FINALdta, aes(x = WeekDate), col = group) + 
  geom_line(data = FINALdta, aes(x = WeekDate, y = Total, color="Dengue Cases")) +
  geom_col(data = FINALdta, aes(x = WeekDate, y = Precip, color="Precipitation")) +
  scale_y_continuous(sec.axis = sec_axis(), name="SOI") +
  xlab("Dates") +
  ylab("Weather Variables") +
  theme_wsj() +
  scale_fill_wsj(palette = "colors6") +
  ggtitle("Weather Variables and Dengue Cases")
print(CP1)

  geom_line(data = FINALdta, aes(x = WeekDate, y = MaxAT + coeff, color = "Average Temp")) +
  geom_line(data = FINALdta, aes(x = WeekDate, y = MinAT + coeff, color = "Average Temp")) +
...
# Compare both
anova(D3.4, D4) #Not usre if I should look at them seperatly in each model or run them together
